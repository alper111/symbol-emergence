...
policy_network.load_state_dict(torch.load("save/tree1/policy_net_300.ckpt"))
value_network.load_state_dict(torch.load("save/tree1/value_net_300.ckpt"))
...
for epi in range(episode):
    ...
    best_expert = policy_network.leaf_probability(obs).argmax()
    actions, _ = policy_network(obs)
    action = actions[best_expert, 0, :actions.shape[-1]//2].detach()
